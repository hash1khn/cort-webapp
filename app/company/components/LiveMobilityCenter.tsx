...
const LiveMobilityCenter = ({ data }: LiveMobilityCenterProps) => {
    ...
    const markers: any[] = [
        // Shuttles
        { id: 'shuttle-1', position: [24.8719, 67.0593], label: 'Shuttle Route 1 - Occupancy 85%', id_type: 'driver' },
        { id: 'shuttle-2', position: [24.9462, 67.1238], label: 'Shuttle Route 4 - Delayed 5m', id_type: 'driver' },
        { id: 'shuttle-3', position: [24.8138, 67.0267], label: 'Shuttle Route 5 - On Track', id_type: 'driver' },
        // Chauffeur rides
        { id: 'car-1', position: [24.8607, 67.0011], label: 'Chauffeur Ride - active', id_type: 'driver' },
        { id: 'car-2', position: [24.8924, 67.0747], label: 'Chauffeur Ride - active', id_type: 'driver' },
        { id: 'car-3', position: [24.8348, 67.0659], label: 'Chauffeur Ride - active', id_type: 'driver' },
        { id: 'car-4', position: [24.9180, 67.0971], label: 'Chauffeur Ride - active', id_type: 'driver' },
        // Employee pickups
    ...
    return (
        <Card className="p-0 overflow-hidden border-none shadow-2xl bg-[var(--cort-navy)] min-h-[600px] flex flex-col rounded-[2rem]">
            {/* Header Area */}
            <div className="m-4 mb-0 p-6 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[var(--cort-navy)] text-white rounded-[2rem] border border-white/5">
                <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md p-4 px-6 rounded-[2rem] border border-white/10">
                ...
                <div className="flex items-center gap-6">
                    <div className="flex flex-col items-end">
                        ...
                        <div className="font-mono text-lg font-bold text-[var(--cort-orange)]">
            ...
            <div className="m-4 mt-2 px-6 py-4 bg-white/5 border border-white/10 rounded-[2rem] grid grid-cols-2 md:grid-cols-5 gap-4 shadow-inner">
                {stats.map((stat, idx) => (
                    <div key={idx} className="flex flex-col">
                        <div className="flex items-center gap-2 text-white mb-1">
                            <span className="p-1 px-1.5 rounded-md bg-white/20 text-[var(--cort-orange)]">{stat.icon}</span>
            ...
            <div className="flex-1 flex flex-col lg:flex-row min-h-[400px]">
                ...
                <div className="flex-1 relative order-2 lg:order-1">
                    <div className="absolute inset-0 p-4">
                        <div className="w-full h-full rounded-[2rem] overflow-hidden border border-white/10 shadow-inner group">
                            <Map
                                height="100%"
                                markers={markers.map(m => ({
                                    ...m,
                                    id: m.id_type // Map.tsx uses id to determine icon type
                                }))}
                                polylines={polylines}
                            ...
                            {/* Map Legend */}
                            <div className="absolute bottom-8 right-8 z-[50] bg-[var(--cort-navy)] backdrop-blur-md border border-white/30 p-4 rounded-2xl shadow-2xl flex flex-col gap-2">
                                <span className="text-[10px] font-black text-white/70 uppercase tracking-widest mb-1 font-mono">Live Legend</span>
                                <div className="flex items-center gap-3">
                                    <div className="w-3 h-3 bg-[#f47f00] rounded-full border border-white/40 shadow-[0_0_12px_rgba(244,127,0,0.8)]"></div>
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-1.5 bg-[var(--cort-orange)] rounded-full shadow-[0_0_12px_var(--cort-orange)]"></div>
                                    <span className="text-xs text-white font-black tracking-tight">Active Routes</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="w-full lg:w-80 m-4 lg:ml-0 p-6 flex flex-col gap-4 bg-[var(--cort-orange)] rounded-[2rem] shadow-2xl order-1 lg:order-2 text-white">
                    <div className="flex items-center justify-between">
                        <h3 className="text-sm font-black uppercase tracking-widest text-[var(--cort-navy)]">Active Alerts</h3>
                        <span className="px-2 py-0.5 rounded-full bg-[var(--cort-navy)]/20 text-[var(--cort-navy)] text-[9px] font-black uppercase border border-[var(--cort-navy)]/10">2 Critical</span>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="p-3 rounded-2xl bg-white/15 border border-white/20 hover:bg-white/25 transition-colors cursor-pointer group">
                            <div className="flex items-start gap-3">
                                <AlertTriangle size={16} className="text-[var(--cort-navy)] mt-1 shrink-0" />
                                <div>
                        ...
                        <div className="p-3 rounded-2xl bg-white/20 border border-white/30 hover:bg-white/30 transition-colors cursor-pointer group shadow-sm">
                            <div className="flex items-start gap-3">
                                <Users size={16} className="text-[var(--cort-navy)] mt-1 shrink-0" />
                                <div>
                        ...
                        {/* Quick Action Button */}
                        <button className="w-full mt-4 py-3 rounded-xl bg-[var(--cort-navy)] hover:bg-[var(--cort-navy-hover)] text-white text-xs font-black uppercase tracking-widest transition-all shadow-lg active:translate-y-0.5">
                    ...
                    <div className="mt-auto pt-6 border-t border-white/10">
                        <div className="p-4 rounded-2xl bg-white/10 border border-white/10">
                            <div className="text-[10px] font-black text-[var(--cort-navy)] uppercase tracking-widest mb-3 opacity-60">Service Performance</div>
                            <div className="space-y-4">
                                <div>
                                    <div className="flex justify-between text-[10px] font-bold text-white mb-1.5 uppercase tracking-tighter">
                                        ...
                                        <span className="text-[var(--cort-navy)] font-black">92%</span>
                                    </div>
                                ...
                                <div>
                                    <div className="flex justify-between text-[10px] font-bold text-white mb-1.5 uppercase tracking-tighter">
                                        ...
                                        <span className="text-[var(--cort-navy)] font-black">78%</span>
                                    </div>
                                    <div className="h-1 w-full bg-white/20 rounded-full overflow-hidden">
